...
const parseEquals = (message, castTo = String) => {
...
const parseSlashToString = (message) => {
	const entries = message.split(",").map((badge) => badge.split("/")).map(([field, value]) => [field, String(value)]);
	return message && Object.fromEntries(entries);
};
const cleanMessage = (message) => message?.replaceAll("\\s", " ");

//#endregion
//#region src/modules/clearchat/parseBan.ts
const parseBan = (fields) => {
...
//#endregion
//#region src/modules/clearchat/parseTimeout.ts
const parseTimeout = (fields) => {
...
//#endregion
//#region src/modules/clearchat/parseClearChat.ts
const parseClearChat = ({ eventMessage }) => {
	...
	else if (fields["target-user-id"]) return {
		...parseBan({
			username,
			channel,
			...fields
...
//#endregion
//#region src/modules/message/parsePrivMsg.ts
const parsePrivMsg = (rawMessage) => {
...
//#endregion
//#region src/modules/message/parseReplyMessage.ts
const parseReplyMessage = (fields) => {
...
//#endregion
//#region src/modules/message/badges/parseBadgeInfo.ts
const BADGES = [
	"subscriber",
	"founder",
	"predictions"
];
const parseBadgeInfo = (badgeInfo) => {
	...
	return { ...fieldsBadgeInfo };
};

//#endregion
//#region src/modules/message/badges/badges.ts
var badges_default = [
	{
...
//#endregion
//#region src/modules/message/parseBadges.ts
const parseBadges = (fields) => {
...
//#endregion
//#region src/modules/message/bots.js
const BOTLIST = [{
	name: "streamelements",
	type: "bot"
}, {
...
//#endregion
//#region src/modules/message/parseUser.ts
const parseUser = (fields) => ({
...
//#endregion
//#region src/modules/message/emotes/createEmotesDictionary.ts
const createEmotesDictionary = (rawMessage) => {
...
//#endregion
//#region src/modules/message/emotes/createFragments.ts
const patterns = [
	{
		type: "nick",
		regex: /@[a-zA-Z0-9_\u3040-\u30FF\u3400-\u9FFF\uAC00-\uD7AF]+/
	},
	{
		type: "hexcolor",
		regex: /#[A-Fa-f0-9]{6}\b/
	...
	{
		type: "laugh",
		regex: /\b(xd+)\b|\b(j(a|e|i|o)*)+\b/i
	}
];
const createFragments = (message) => {
	...
	for (const match of message.matchAll(combinedRegex)) {
		...
		if (index > lastIndex) fragments.push({
			type: "text",
			data: message.slice(lastIndex, index)
		});
		const matchedPattern = patterns.find((p) => new RegExp(`^${p.regex.source}$`, p.regex.flags).test(matchStr));
		fragments.push({
	...
	if (lastIndex < message.length) fragments.push({
		type: "text",
...
//#endregion
//#region src/modules/message/emotes/createEmoteImage.ts
const createEmoteImage = (id, format = "default", theme = "dark", scale = "2.0") => ({
	type: "emote",
...
//#endregion
//#region src/modules/message/emotes/parseMessageWithEmotes.ts
const parseMessageWithEmotes = (fields) => {
	...
	emoteList.forEach(({ name, start, end }) => {
		if (!emoteOnly) newMessage.push(...createFragments(rawMessage.substring(i, start)));
...
//#endregion
//#region src/modules/message/parseFlags.ts
const parseFlags = (fields) => {
...
//#endregion
//#region src/modules/message/emotes/createMessageNode.ts
const createMessageNode = (messageData) => messageData.map((fragment) => {
	if (fragment.type === "emote") return `:${fragment.id}:`;
	else return fragment.data;
}).join("");

//#endregion
//#region src/modules/message/emotes/createMessageBrowser.ts
...
const createMessageBrowser = (messageData) => {
	const elements = messageData.map((fragment) => {
		if (fragment.type === "emote") return createImage(fragment);
		else if (fragment.type === "7tv") return create7tvImage(fragment);
		else return createSpan(fragment, fragment.type);
	});
	const span = document.createElement("span");
...
//#endregion
//#region src/modules/message/emotes/createMessage.ts
const createMessage = (() => {
...
//#endregion
//#region src/modules/message/parseMessage.ts
const parseMessage = (fields) => {
	const { username, channel, flags, rawMessage } = fields;
	const messageData = parseMessageWithEmotes({
		rawMessage,
		...fields
	});
	const spanMessage = createMessage(messageData);
	const flagsInfo = parseFlags({
		flags,
		rawMessage
	});
	return {
...
//#endregion
//#region src/modules/message/parseBits.ts
const parseBits = (fields) => {
...
//#endregion
//#region src/modules/message/parseUserMessage.ts
const parseUserMessage = ({ eventMessage }) => {
	...
	const messageInfo = parseMessage({
		username,
		channel,
		rawMessage,
		...fields
	...
	if (Object.keys(bitsInfo).length > 0) return {
		...data,
		bitsInfo,
		type: "bits"
	};
...
//#endregion
//#region src/modules/joinpart/parseJoinPart.ts
const parseJoinPart = ({ eventMessage }) => {
...
//#endregion
//#region src/modules/clearmsg/parseClearMsg.ts
const parseClearMsg = ({ eventMessage }) => {
...
//#endregion
//#region src/modules/usernotice/sub/parseSubPlan.ts
const parseSubPlan = (fields) => {
...
//#endregion
//#region src/modules/usernotice/sub/parseSub.ts
const parseSub = (fields) => {
...
//#endregion
//#region src/modules/usernotice/gift/parseMysteryGift.ts
/**
* El usuario senderUsername regala massGiftCount subs a la comunidad.
*/
const parseMysteryGift = (fields) => {
...
//#endregion
//#region src/modules/usernotice/gift/parseStandardPayforward.ts
const parseStandardPayforward = (fields) => {
...
//#endregion
//#region src/modules/usernotice/gift/parseCommunityPayforward.ts
const parseCommunityPayforward = (fields) => {
	const gifterAnonymous = fields["msg-param-prior-gifter-anonymous"] !== "false";
	return {
		gifterAnonymous,
		gifterId: Number(fields["msg-param-prior-gifter-id"]),
...
//#endregion
//#region src/modules/usernotice/gift/parseGiftPaidUpgrade.ts
const parseGiftPaidUpgrade = (fields) => {
...
//#endregion
//#region src/modules/usernotice/gift/parsePrimePaidUpgrade.ts
const parsePrimePaidUpgrade = (fields) => {
	const username = fields.login;
	return {
		type: fields["msg-id"],
		username,
...
//#endregion
//#region src/modules/usernotice/gift/parseGoal.ts
const parseGoal = (fields) => {
	if (!fields["msg-param-goal-contribution-type"]) return {};
	return {
...
//#endregion
//#region src/modules/usernotice/gift/parseSubGift.ts
const parseSubGift = (fields) => {
	const originId = cleanMessage(fields["msg-param-origin-id"]);
	const subPlan = parseSubPlan(fields);
...
//#endregion
//#region src/modules/usernotice/parseGift.ts
const parseGift = (fields) => {
	const msgId = fields["msg-id"];
	if (msgId === "submysterygift") return parseMysteryGift(fields);
	if (msgId === "standardpayforward") return parseStandardPayforward(fields);
	if (msgId === "communitypayforward") return parseCommunityPayforward(fields);
	if (msgId === "primepaidupgrade") return parsePrimePaidUpgrade(fields);
	if (msgId === "giftpaidupgrade") return parseGiftPaidUpgrade(fields);
	if (msgId === "subgift") return parseSubGift(fields);
	if (!fields["msg-param-sub-plan"]) return {};
	return {};
};

//#endregion
//#region src/modules/usernotice/parseRaid.ts
const parseRaid = (fields) => {
...
//#endregion
//#region src/modules/usernotice/parseAnnouncement.ts
const parseAnnouncement = (fields) => {
	if (fields["msg-id"] !== "announcement") return {};
...
//#endregion
//#region src/modules/usernotice/parseViewerMilestone.ts
const parseViewerMilestone = (fields) => {
...
//#endregion
//#region src/modules/usernotice/parseUserNotice.ts
const parseUserNotice = ({ eventMessage }) => {
...
//#endregion
//#region src/modules/parseRawMessage.ts
const parseRawMessage = ({ eventMessage }) => {
	const [host, id, user] = eventMessage.split(" ", 3);
	eventMessage.split(" ").slice(3).join(" ").substring(1);
...
//#endregion
//#region src/modules/notice/parseNotice.ts
const parseNotice = ({ eventMessage }) => {
	const [rawFields, host, rawType, channel, ...rawMessage] = eventMessage.substring(1).split(" ");
	const fields = parseEquals(rawFields);
	const message = rawMessage.join(" ").substring(1);
...
//#endregion
//#region src/modules/roomstate/parseRoomState.ts
const parseRoomState = ({ eventMessage }) => {
...
//#endregion
//#region src/debugId.ts
const IGNORETYPES = ["PRIVMSG", "ROOMSTATE"];
const IGNORELIST = [
	"resub",
	"sub",
	"subgift",
...
var Client = class {
	...
	#manageEvent(eventData) {
		...
		if (["join", "part"].includes(eventType) && eventData.username && eventData.username.startsWith("justinfan")) return;
		this.#done = true;
	...
	#close(event) {
		...
		console.log(`${type}: REASON ${reason} ${code}`);
		this.close();
		if (code === 1006) {
